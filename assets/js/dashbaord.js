 <script>
        document.addEventListener("DOMContentLoaded", function () {
            ...
            function switchPanel(panelId) {
                ...
                if (exportBtn) {
                    if (panelId === 'analytics') {
                        ...
                        exportBtn.onclick = function () { alert('📊 Exporting analytics data as CSV...'); };
                    } else {
                ...
                if (actionBtn) {
                    ...
                    if (panelId === 'dashboard') {
                        actionBtn.onclick = function () { alert('📊 Dashboard overview exported!'); };
                        actionBtn.removeAttribute('data-bs-toggle');
                        actionBtn.removeAttribute('data-bs-target');
                    } else if (panelId === 'analytics') {
                        actionBtn.onclick = function () { alert('📊 Exporting analytics data...'); };
                        actionBtn.removeAttribute('data-bs-toggle');
                        actionBtn.removeAttribute('data-bs-target');
                    } else if (panelId === 'settings') {
                        actionBtn.onclick = function () { alert('💾 Settings saved!'); };
                        actionBtn.removeAttribute('data-bs-toggle');
                        actionBtn.removeAttribute('data-bs-target');
                    } else {
                        actionBtn.setAttribute('data-bs-toggle', 'modal');
                        actionBtn.setAttribute('data-bs-target', '#addModal');
                        const mode = panelId === 'users' ? 'user' : (panelId === 'orders' ? 'order' : 'report');
            ...
            window.openAddModal = function (mode) {
                let fields = '';
                let title = 'Add New';
            ...
            addForm.addEventListener('submit', function (e) {
                ...
                switch (mode) {
                    case 'user':
                        ...
                        document.getElementById('statNewUsers').textContent = (parseInt(document.getElementById('statNewUsers')
                            .textContent) || 0) + 1;
                        const tbody = document.getElementById('userTableBody');
                        const row = document.createElement('tr');
                        const initials = name.split(' ').map(n => n[0]).join('').toUpperCase();
                        row.innerHTML =
                            `<td><div class="d-flex align-items-center gap-2"><span class="user-avatar-sm" style="background:var(--gradient-prime);">${initials}</span> ${name}</div></td>
                    ...
                    case 'order':
                        ...
                        document.getElementById('statPendingOrders').textContent = (parseInt(document.getElementById(
                            'statPendingOrders').textContent) || 0) + 1;
